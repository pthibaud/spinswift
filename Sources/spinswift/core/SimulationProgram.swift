...
class SimulationProgram: Codable {
    ...
    struct Inputs: Codable {
        ...
        init(
            T_initial: Double? = Double(), T_step: Double? = Double(), T_final: Double? = Double(),
            time_step: Double? = Double(), stop: Double? = Double(), α: Double? = Double(),
    ...
    func simulate(program: String? = nil, inputs: Inputs) {
        ...
        case "curie_temperature":
            curieTemperature(inputs: inputs)
        case "optical_pulse":
            opticalPulse(inputs: inputs)
        case "time_dynamics":
            timeDynamics(inputs: inputs)
    ...
    private func curieTemperature(inputs: Inputs) {
        var T: Double = inputs.T_initial
        var content1: String = String()
        var content2: String = String()
        let stop: Double = inputs.stop
        let Δt: Double = inputs.time_step
        let T_final: Double = inputs.T_final
        let dT: Double = inputs.T_step
        let α: Double = inputs.α
        let thermostat: String = inputs.thermostat
        var sl1: [Atom] = []
        var sl2: [Atom] = []

        for i in I.h.atoms {
            if i.type == 1 { sl1.append(i) } else { sl2.append(i) }
        }

        while T < T_final {
            let Fn: String = "CT_T_" + String(format: "%.0f", T)
            I.evolveRK45(
                stop: stop, Δt: Δt, fileName: Fn, Temp: T, alpha: α, thermostat: thermostat)
            ...
            content1 +=
                String(T) + "\t" + String(m.x) + "\t" + String(m.y) + "\t" + String(m.z) + "\t"
                + String(mnorm) + "\t"
            content1 += String(χ.xx) + "\t" + String(χ.yy) + "\t" + String(χ.zz) + "\t"
            content1 += String(χ.xy) + "\t" + String(χ.yz) + "\t" + String(χ.zx) + "\n"

            content2 +=
                String(T) + "\t" + String(sl1[1].g * mnorm1) + "\t" + String(sl2[1].g * mnorm2)
                + "\t" + String(mnorm) + "\t"
    ...
    private func opticalPulse(inputs: Inputs) {
        let pulse = LaserExcitation.Pulse(
            Form: "Gaussian", Fluence: 32.5, Duration: 60E-15, Delay: 5e-12)
        ...
        while laser.CurrentTime < inputs.stop * 1E-12 {
            ...
            for a in I.h.atoms {
                a.advanceMoments(
                    method: "rk4", Δt: Δt, T: laser.temperatures.Electron, α: inputs.α,
                    thermostat: inputs.thermostat)
            }
            ...
            content +=
                String(laser.CurrentTime) + "\t" + String(m1.z) + "\t" + String(m2.z) + "\t"
                + String(m.z) + "\t"
            content +=
                String(laser.ComputeInstantPower(time: laser.CurrentTime)) + "\t"
                + String(laser.temperatures.Electron) + "\t" + String(laser.temperatures.Phonon)
                + "\t" + String(laser.temperatures.Spin) + "\n"
    ...
    private func timeDynamics(inputs: Inputs) {
        let T: Double = inputs.T_initial
