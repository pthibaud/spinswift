...
/// A class for managing periodic table for atomic properties
/// 
...
class PeriodicTable {
    ...
    */
    ...
    let Z_short: [AnyHashable: String] = [
    ...
    func radius(_ key: String, method: String? = "Empirical") -> Int {
         switch method!.capitalized {
            case "Vdw":
                return radius_vdw[Z(key)] ?? -1
            case "Calc":
                return radius_calc[Z(key)] ?? -1
            case "Empirical":
                return radius_empirical[Z(key)] ?? -1
            default:
                return radius_empirical[Z(key)] ?? -1
        } 
    }
