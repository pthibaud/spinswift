...
class Interaction: Codable {
    ...
    func demagnetizingField(_ nx: Double, _ ny: Double, _ nz: Double) -> Interaction {
        ...
        atoms.forEach {
            $0.ω -= Vector3(
                nx * ($0.moments.spin.x), ny * ($0.moments.spin.y), nz * ($0.moments.spin.z))
    ...
    func update() {
        ...
        if isSTT_Damping.computed {
            self.STT_DampingLike(Pol: isSTT_Damping.Pol, Amplitde: isSTT_Damping.Amplitde)
        }
